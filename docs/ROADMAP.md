...
- ✅ **SLA module — config + live timers + dashboard** (Pass 5 chunk 4): migration `0005_sla_policies.sql` for `sla_policies` (org-wide default + per-client overrides, role-gated writes); `SlaPolicy` / `SlaTier` / `SlaIncidentSnapshot` types; store slice with `addSlaPolicy` / `updateSlaPolicy` / `removeSlaPolicy` ops + dual-write + hydration; `lib/automation/sla.ts` pure-function evaluator (resolves the right policy by client → org default, computes response + resolution deadlines from `task.createdAt`, classifies as `breached` / `at_risk` / `ok` / `met` from current task state, supports business-hours and 24/7 counting); Settings → SLA policies panel (create / edit dialog with priority tiers grid, escalation user picker, hours-kind toggle); new `/sla` dashboard page (stat tiles, state filter chips, group-by-client task table with response + resolution chips and "next deadline" countdown, deep-link into the task drawer); `<SlaChip />` in the task header (drawer / kanban detail); sidebar nav entry. Escalation emails on breach are deferred to Pass 7's notification engine — recipients persist on the policy.

- ✅ **Visual automation builder** (Pass 5 chunk 3): `<AutomationBuilder />` dialog with three-step flow — trigger picker (9 trigger types with descriptions + per-trigger meta config like target status / budget threshold), optional condition rows (free-form key/value, persisted for future engine work), action picker (9 action types, per-action meta fields like Slack channel / target user / target status / webhook URL). Live preview chip strip at the bottom. New `addAutomation` / `updateAutomation` / `removeAutomation` store ops with dual-write to `automations` + activity log entries. "New rule" button + per-rule pencil-edit + trash-delete on the automations page now do something instead of being decorative.
...
- 🟡 SLA module (Pass 5 chunk 4): config + live timers + dashboard ✅; escalation chain emails on breach ⛔ (recipients persisted, send needs an evaluator-driven hook); per-client SLA report ⛔ (covered by the existing dashboard's groupBy-client view, but no historical/exportable report yet)
- ✅ **Custom report builder** (Pass 5 chunk 5): migration `0006_custom_reports.sql`; `CustomReport` / `ReportConfig` / `ReportFilter` types; store slice + add/update/remove ops with dual-write + hydration; `lib/reports/runner.ts` pure runner with field catalog per source (tasks / time entries / projects / invoices / clients), filter operators (eq/neq/gt/gte/lt/lte/in/contains), measure kinds (count / sum / avg), group + sort + limit; `<ReportBuilder />` dialog with linear flow (data source → group + measure → filters → visual → live preview) and three visuals (table / horizontal-bar / kpi tile); the dead "Custom report" button is now wired and "Export PDF" triggers the browser's print dialog. Saved reports appear as a grid of cards on `/reports` with edit + delete + auto-recompute against live store data on every render.
